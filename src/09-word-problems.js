 ...
 *
 * Write an algorithm that will determine the price of a ticket based on the `priceInCents` of the ticket, the `age` of the attendee, and the membership status 
 * (i.e. `hasMembership`).
...
function applyDiscount(priceInCents, age, hasMembership) {
  ...
  return discount > 0 ? priceInCents - (priceInCents * discount) : priceInCents;
}
 ...
 * 
 * Write an algorithm that will determine the total amount of all items in the cart. Make sure to multiply the `priceInCents` times the `quantity` to get the full 
 ...
 * 
 * Write an algorithm that takes in two objects of similar shape, each object representing an address. Then, 
 * return a string that describes the relationship between those two addresses.
 * 
 ...
 *
 * - If the assignment has a `kind` of `"PASS-FAIL"`, set the `status` value to 
 * `"PASSED"` if the `score.received` equals the `score.max`. Otherwise, set that 
 * `status` to be `"FAILED"`.
 * - If the assignment has a `kind` of `"PERCENTAGE"`, set the `status` value to be 
 * `"PASSED: <percentage>"` if the student scored at least 80.0%. The `<percentage>`
 *  should be set to one decimal place. If the student scored less than 80.0%, set
 *  the status to `"FAILED: <percentage>"`.
 * - If the assignment has any other `kind` than the two above, set the `status`
 *  value to equal `"SCORE: <received>/<max>"`, where `<received>` is the 
 * `score.received` value and `<max>` is the `score.max` value.
 *
...
function gradeAssignments(assignments) {
  for (const iterator of assignments) {
    switch(iterator.kind)
    {
      case "PASS-FAIL":
      iterator.status = iterator.score.received === iterator.score.max ?
       "PASSED" : "FAILED";
      break;

      case "PERCENTAGE":
      let score = (100*(iterator.score.received/iterator.score.max)).toFixed(1);
      iterator.status = (iterator.score.received/iterator.score.max)*100 >= 80 ? 
      `PASSED: ${score}%` : `FAILED: ${score}%`;
      break;

      case "ESSAY":
      iterator.status = `SCORE: ${iterator.score.received}/${iterator.score.max}`;
    }
  }
  return assignments;
}
 ...
 * createLineOrder()
 * ---------------------
 * An airline wants to build an application that improves the boarding process for
 *  its customers. They want to have customers sign up in order of arrival, 
 * but prioritize those customers who have a membership.
 * 
 * Build an algorithm that takes in an array of objects, where each object 
 * represents a person. The order of the array is important; the person at 
 * index `0` arrived first while the person at index `1` arrived afterwards.
 * 
 * Return an array that includes only the names of each person, but reordered 
 * to account for whether or not each person has a membership. Everyone who has 
 * a membership should be at the front of the line in the same order they arrived. 
 * Everyone without a membership should be in the same order they arrived but after
