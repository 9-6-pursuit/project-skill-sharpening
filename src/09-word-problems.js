...
function getCartTotal(products) {
  ...
  for(i = 0; i < products.length; i++){
    totalPrice += products[i].priceInCents *
    products[i].quantity
  }
...
function gradeAssignments(assignments) {
  ...
  for (let i = 0; i < assignments.length; i++) {
      //assignments.assignments[i].status
    ...
    if (assignments[i].kind === "PERCENTAGE" && assignments[i].score.received/assignments[i].score.max >= .8) {
