...
function applyDiscount(priceInCents, age, hasMembership) {
  ...
  let total = priceInCents * (1 - discount)
  return total
...
function getCartTotal(products) {
  ...
  for (const val of products) {
    total += val.priceInCents * val.quantity
  }
  let newNum = (total/100).toFixed(2)
  return `$${newNum}`
}
...
function gradeAssignments(assignments) {
  for (const val of assignments) {
    switch(val.kind){
      case "PASS-FAIL":
        val.score.received === val.score.max ? val.status = 'PASSED' : val.status = 'FAILED'
      break;
      case "PERCENTAGE":
        let percent = (val.score.received/val.score.max) * 100
        if(percent >= 80.0){
          val.status = `PASSED: ${percent.toFixed(1)}%`
        } else {
          val.status = `FAILED: ${percent.toFixed(1)}%`
        }

      break;
      default:
        val.status = `SCORE: ${val.score.received}/${val.score.max}`
    }
  }
  return assignments
}
...
function createLineOrder(people) {
  ...
  for (const val of people) {
    if (val.hasMembership === true){
      newString.push(val.name)
    }
  }
  for (const val of people) {
    if (val.hasMembership === false){
      newString.push(val.name)
    }
  }
  return newString
