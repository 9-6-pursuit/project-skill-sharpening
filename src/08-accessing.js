...
function getLastPhoneNumber(person) {
  ...
  // let lastNumber = person.numbers.pop()
  // return lastNumber
}
...
function getFullAddress(person) {
  ...
  let addressUnit = person.address.unit
  let addressCity = person.address.city
  let addressState = person.address.state
  let addressZip = person.address.zip
  
  return `${addressStreet} ${addressUnit}, ${addressCity}, ${addressState}, ${addressZip}`;
}
