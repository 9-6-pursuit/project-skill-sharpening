...
function getLastPhoneNumber(person) {
  if (person.numbers.length > 1){
    return Number(person.numbers.slice(-1))
  }
  else {
    return Number(person.numbers)
  }
}
...
function getFullAddress(person) {
  let str = person.address.street + " "+ person.address.unit
  let cty = person.address.city
  let st = person.address.state
  let zp = person.address.zip

  return str + ", "+ cty + ", " + st + ", " + zp
}
