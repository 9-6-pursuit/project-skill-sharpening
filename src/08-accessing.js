...
function getCountOfPhoneNumbers(person) {
  let digits = 0
  for(let properties in person.numbers){
    digits = digits + 1
  }
  return digits
}
...
function getCityAndState(person) {
  ...
  let personState = person.address.state
  return personCity + "," + " " + personState
}
...
function getFullAddress(person) {
  ...
  let personSuite = person.address.unit
  let personCity = person.address.city
  let personState = person.address.state
  let personZip = person.address.zip
  return personStreet + " " + personSuite + "," + " " + personCity + "," + " " + personState + "," + " " + personZip
...
function getFlatObject(person) {
   ...
   result.first = person.names.first
   result.middle = person.names.middle
   result.last = person.names.last

   result.street = person.address.street
   result.unit = person.address.unit
   result.city = person.address.city
   result.state = person.address.state
   result.zip = person.address.zip
